...
> **当前下载：0.3.0** · [查看发行说明](https://github.com/Lincb522/GLEVIO/releases/tag/v0.3.0)
>
> 下一版 0.4.1 的改进见下方，尚未包含在当前安装包中。
...
以下改进已进入开发版本，安装包尚未公开发布。新交互仍在验证中。
...
已有文件夹和隐藏应用会保留。仅传递分类所需的应用名称、标识和类别信息；CLI 会使用你配置的模型，可能连接云端服务，并不代表完全离线。生成过程可停止，未得到有效建议时仍可使用内置规则整理。
